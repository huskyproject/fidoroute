...
static const char * ErrLoop    =
    EOLCHR "RouteLoop detected for %s. Try to route by default";
...
enum
{
    DIRECT_FLAVOR = 1, CRASH_FLAVOR = 2, HOLD_FLAVOR = 4, NORMAL_FLAVOR = 16, FILE_FLAVOR = 32,
    NOARC_FLAVOR  = 64
};
...
static ushort TestCmpQuality(nodeaddr & S, nodeaddr & D)
{
    ...
    if((D.z != WILDVALUE && S.z != D.z) || (D.n != WILDVALUE && S.n != D.n) ||
...
const char * SqFlavors[] = {"Crash ", "Direct", "Hold  ", "Normal"};
...
static void PutDownLinksGeneric(ushort UpIdx, ushort type, ushort (* wrtnode)(nodeaddr,
                                                                              char *,
                                                                              ushort))
{
    ushort i;
    ...
    for(i = 0; i < nNodes; i++)
    {
        if(Node[i].idx == UpIdx && !DirectLink(i) && !IsMyNode(Node[i].addr) &&
           ((UpIdx != WILDVALUE && Node[i].idx != i) || (UpIdx == WILDVALUE && Node[i].is_uplink)))
        {
...
static const char * RouteType[] = {"Nodes", "Nets", "Zones", "Default"};
...
const char * ItrFlavors[] = {"Crash", "Dir  ", "Hold ", "     "};
...
const char * BipFlavors[] = {"Crash", "Dir  ", "Hold ", "Norm "};
...
static void PutRoutingQecho(void)
{
    ...
    for(level = 0; level < 3; level++)
    {
        ...
        for(i = 0; i < nLinks; i++)
        {
            sprintf(Prefix, "RouteVia\t%d:%d/%d@fidonet" EOLCHR "RouteFor\t", Link[i].addr.z,
                    Link[i].addr.n, Link[i].addr.f);
            strcpy(Buff, Prefix);
...
const char * FidogateFlavors[] = {"crash ", "direct", "hold  ", "normal"};
...
static void PutRoutingFtrack(void)
{
    ...
    for(level = 0; level < 4; level++)
    {
        ...
        for(i = 0; i < nLinks; i++)
        {
            tmpNode = Link[i].addr;
            ushort isHost =
                (DirectLink(tmpNode) && BestAKALevel(tmpNode) == 1 && tmpNode.f == WILDVALUE);
...
const char * HuskyFlavors[] = {"crash", "direct", "hold", "normal"};
...
static void PutUnRouted(void)
{
    ...
    FullAddr  = true;
    fprintf(NewRoute,
            "%c" EOLCHR "%c !!! Undefined !!!" EOLCHR "%c ----------------" EOLCHR "",
            CmntSym,
            CmntSym,
            CmntSym);
    level     = 0;
...
static boolean GetBoolean(char * p, void * target)
{
    ...
    else if(strnicmp(p, "no", 2) == 0 || strnicmp(p, "off", 3) == 0 || strnicmp(p, "0", 1) == 0)
    {
        *(ushort *)target = 0;
    }
    else
            ...
            while(fgets(Buff, BUFFLEN, ndl) != NULL)
            {
                ...
                if(Buff[0] != ';')
                {
                    ...
                    else if((found & 0x03) == 3)
                    {
                        ...
                        else if((strnicmp(Buff, "hold",
                                          4) == 0) || (strnicmp(Buff, "down", 4) == 0))
                        {
...
{   {"Address",       NULL,           1,
    ...
    {"TrustFile",     NULL,           4,
     GetTrustFile                                                 },
    {"RouteType",     &RouteMode,     1,
     GetRouteType                                                 },
    {"WriteTo",       WriteTo,        2,
     GetDestFile                                                  },
    {"Minimize",      &MinMode,       1,
     GetBoolean                                                   },
    {"KillTransit",   &KillTransit,   1,
     GetBoolean                                                   },
    {"DefaultFlavor", &DefaultFlavor, 2,
     GetFlavor                                         },
    {"DefaultRoute",  NULL,           5,
     GetRouteStr                                                  },
    {"Link",          NULL,           2,
     GetLink                                           },
    {"RouteBegin",    RouteBegin,     1,
     GetQuotedString                                   },
    {"RouteEnd",      RouteEnd,       1,
     GetQuotedString                                   },
...
int main(int argc, char ** argv)
{
    ...
    if(Buff == NULL || Prefix == NULL || Node == NULL || Link == NULL || CfgFile == NULL ||
       MyNode == NULL)
    {
        Error(ErrNoMemory);
        return 2;
